...
def harmonic_interruption_generator():
    for i in range(3601):
        ...
        if os.path.exists(ruta_archivo):
            ...
            # Sumar la señal armónica y la señal con interruption
            signal_with_harmonic = signal_original + harmonic_signal
            signal_with_harmonic_and_interruption = np.copy(signal_with_harmonic)
            signal_with_harmonic_and_interruption[inicio_amplificacion:(inicio_amplificacion + duracion_amplificacion)] *= (
                factor_amplitud)
            ...
            nombre_archivo_nuevo = f"hrc_int_s_{i}.npy"
...
harmonic_interruption_generator()
